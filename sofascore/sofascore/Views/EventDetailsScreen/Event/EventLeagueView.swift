...
class EventLeagueView: BaseView {
    ...
    var arrowTap: (() -> Void)?
    var leagueTap: ((League, String) -> Void)?
    var league: League?
    var sport: String?
    ...
    override func styleViews(){
        ...
        leagueText.font = Fonts.RobotoCondensedRegularTitle12
        leagueText.textColor = Colors.leagueInfoColor
        
        leagueCell.isUserInteractionEnabled = true
    ...
    override func setupGestureRecognizers(){
        let back = UITapGestureRecognizer(target: self, action: #selector(didTapArrow))
        let league = UITapGestureRecognizer(target: self, action: #selector(didTapLeague))
        
        arrow.addGestureRecognizer(back)
        leagueCell.addGestureRecognizer(league)
    }
