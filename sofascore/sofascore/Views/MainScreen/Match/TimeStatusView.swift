...
class TimeStatusView: BaseView {
    ...
    override func styleViews(){
        ...
        matchStartCell.font = Fonts.RobotoCondensedRegularTitle12
        matchStartCell.textColor = Colors.matchStartColor
        matchStartCell.textAlignment = .center
        
        minuteCell.font = Fonts.RobotoCondensedRegularTitle12
    ...
    func timeStatusInfoLeague(startTimestamp: Int, status: EventStatus){
        matchStartCell.text = Helper.matchDateShort(startTimestamp: startTimestamp)
        minuteCell.text = Helper.minute(startTimestamp: startTimestamp, status: status)
        minuteCell.textColor = Helper.minuteColor(status: status)
    }
}
